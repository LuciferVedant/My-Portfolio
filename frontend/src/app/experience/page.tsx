...
export default function ExperiencePage() {
  return (
    <div className="max-w-4xl mx-auto px-6 py-8 space-y-12">
      ...
      {/* Experience Timeline */}
      <div className="relative border-l border-indigo-500/30 pl-6 sm:pl-8 ml-2 sm:ml-4 space-y-12">
        {experiences.map((exp, idx) => (
          ...
          >
            {/* Timeline Dot */}
            <div
              className={`absolute -left-[31px] sm:-left-[39px] top-1.5 w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                exp.isCurrent
                  ? 'bg-indigo-600 border-white shadow-lg shadow-indigo-500/50'
                  : 'bg-slate-900 border-indigo-500'
              }`}
            >
              {exp.isCurrent && <div className="w-2 h-2 rounded-full bg-white animate-ping" />}
            </div>
            ...
            <div className="glass-card p-6 sm:p-8 rounded-2xl space-y-4">
              ...
              {/* Tech Badges */}
              <div className="pt-3 flex flex-wrap gap-1.5 border-t border-slate-800/60">
                {exp.tech.map((t, tIdx) => (
                  <span
                    key={tIdx}
                    className="px-2.5 py-1 rounded-md bg-slate-900 border border-slate-700/60 text-[11px] font-mono text-indigo-300"
                  >
                    {t}
                  </span>
                ))}
